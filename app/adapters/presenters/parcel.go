...
func ParcelsResponse(data *PaginationResponse) *fiber.Map {
	...
	for _, p := range data.Data.([]*ent.Parcel) {
		response = append(response, &ParcelResponseData{
			...
			Transaction: func() *TransactionResponseData {
				if t, err := p.Edges.TransactionOrErr(); err == nil && t != nil {
					return &TransactionResponseData{
						Reference:       t.Reference,
						TransactionType: string(t.Channel),
						Amount:          t.Amount,
